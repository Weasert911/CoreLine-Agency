# _**CLIENTS**_

|ID|Name|Manager|Acquisition Channel|Lead Source|Start Date|Contract Type|Monthly Retainer|Status|Country|Industry|
|---|---|---|---|---|---|---|---|---|---|---|
|C1|revoeditzx|Manab|Discord|Organic|2026-02-20|Project|0|Active|India|Creator|

**Status:**
- Active
- Inactive
- Prospect
- Churned
**Contract Type:**
- Project
- Monthly
- Performance
---

# _**LEADS**_

|Lead ID|Date|Name|Platform|Service Interested|Estimated Value|Assigned To|Status|
|---|---|---|---|---|---|---|---|
|L1|2026-02-23|revoeditzx|Discord|Editing|17|Ritesh|Closed|
|L2|2026-02-23|KJ-29|Reddit|Editing|0|Ritesh|Lost|

**Status:**
- Open
- Contacted
- Closed
- Lost
---
# _**DEALS**_

|Deal ID|Date|Client ID|Lead ID|Outreacher|Manager|Editor|Service Type|Quantity|Amount|Editor Cost|Other Cost|Discount|Tax|Payment Status|Deal Status|Payment Date|
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
|D1|2026-02-23|C1|L1|Ritesh|Manab|Cypher|Editing|1|17|10|0|0|0|Paid|Closed|2026-02-23|

**Deal Status:**
- Open
- Closed
- Lost
- Refunded

**Payment Status:**
- Pending
- Paid
- Partial
- Refunded

---

# _**EXPENSES**_

|Date|Category|Subcategory|Description|Amount|Type|
|---|---|---|---|---|---|

**Type:**
- Fixed
- Variable
---

# _**TEAM**_

|Name|Role|Commission %|Base Salary|Join Date|Status|
|---|---|---|---|---|---|
|Ritesh|Growth and Sales Admin|25|0|2026-01-17|Active|
|Priyansh|Technical Admin|25|0|2026-01-17|Active|
|Manab|Worker's Manager|10|0|2026-02-21|Active|
|Veer|Records & Process Manager|10|0|2026-02-21|Active|

**Status:**
- Active
- Inactive
---

# _**SOP TRACKER**_

|SOP ID|Department|Title|Created By|Created Date|Status|Last Updated|Notes|
|---|---|---|---|---|---|---|---|

**Status:**
- Draft
- Active
- Needs Update
- Archived
---

# _**PROCESS IMPROVEMENTS**_

|ID|Date|Department|Problem Identified|Solution Proposed|Implemented (Yes/No)|Impact Level|
|---|---|---|---|---|---|---|

**Impact Level:**
- Low
- Medium
- High
---
# _**FREELANCER PERFORMANCE**_

|Date|Freelancer|Project/Deal ID|On-Time (Yes/No)|Revision Count|Quality Rating (1-5)|Notes|
|---|---|---|---|---|---|---|

---
